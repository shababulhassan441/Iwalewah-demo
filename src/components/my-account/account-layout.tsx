...
const accountMenu = [
  ...
  // },
  {
    slug: ROUTES.WHOLESALE_REQUEST, // '/my-account/help-center'
    name: 'text-account-details-help', // Currently mapped to 'Help Center'
...
const AccountLayout: React.FunctionComponent<{}> = ({ children }) => {
  const [menuItems, setMenuItems] = useState(accountMenu);
      ...
      setMenuItems(
        accountMenu.filter(
          (item) =>
            ...
            (item.slug !== ROUTES.ACCOUNT_SETTING &&
              item.slug !== ROUTES.CHANGE_PASSWORD && item.slug !== ROUTES.WHOLESALE_REQUEST)
        )
      );
    }
  };
