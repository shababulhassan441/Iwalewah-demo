...
interface ListBoxProps {
  options: Option[];
  onChange: (value: string) => void; // New onChange prop from the parent
}

export default function ListBox({ options, onChange }: ListBoxProps) {
  const { t } = useTranslation('common');
  const router = useRouter();
  const { pathname, query } = router;

  // Get the current selected item from the URL query or default to the first option
  ...
  // Update the selectedItem state if the query changes
  useEffect(() => {
    setSelectedItem(currentSelectedItem);
  }, [currentSelectedItem]);

  // Call onChange whenever an item is clicked
  function handleItemClick(values: Option) {
    setSelectedItem(values);
    onChange(values.value); // Pass the selected value to the parent
